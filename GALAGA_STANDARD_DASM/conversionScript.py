...
def convert():
    ...
    input_path = os.getcwd() + os.sep + input_path
    output_path = os.getcwd() + os.sep + output_path
    heading_path = os.getcwd() + os.sep + heading_path
    ...
    for root, dirs, files in os.walk(input_path):
        for file in files:
            ...
            if input_extension in file:
                ...
                copyfile(root + os.sep + file, output_path + os.sep + file[:-2] + output_extension)

                file_in = open(root + os.sep + file, 'r')
                file_out = open(output_path + os.sep + file[:-2] + output_extension, 'w')
                log_out = None

                if log:
                    log_out = open(output_path + os.sep + file[:-2] + "_log.txt", 'w')
